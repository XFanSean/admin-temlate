<template>
  <div ref="el" class="the-scrollbar" @mouseenter="onEnter()" @mouseleave="onLeave()">
    <div ref="wrap" :style="wrapStyle" class="the-scrollbar-wrap">
      <slot></slot>
    </div>
...
import { onMounted, ref, reactive, onUnmounted, watch, watchEffect } from 'vue'
...
function updateThumbStyle() {
  ...
  if (wrapEl) {
    ...
    thumbStyle.y.transform = `translate3d(0, ${
      (wrapEl.scrollTop / wrapEl.scrollHeight) * wrapEl.clientHeight
    }px, 0)`
    ...
    thumbStyle.x.transform = `translate3d(${
      (wrapEl.scrollLeft / wrapEl.scrollWidth) * wrapEl.clientWidth
    }px, 0, 0)`
    // console.log("------------------------------------");
  }
}
